...
public sealed partial class CompositeQuery
{
    ...
    /// </returns>
    public IEnumerable<MemberBinding> CreateIterRowBindings(ParameterExpression itorRowVariable, Type sourceType, Type targetType)
    {
        ...
        foreach (var sourceProperty in sourceProperties)
        {
            ...
            {
                var sourceValue = Expression.Property(itorRowVariable, "Item", Expression.Constant($"{alias}_{sourceProperty.Name}"));
    ...
    /// expression trees. This method iterates over the input data, constructs entities of type <typeparamref name="TEntity"/>,
    /// and accumulates them into the output collection based on a set of processor expressions defined in <see cref="IterRowProcessors"/>.
    /// The processing logic is built dynamically and executed via a compiled lambda expression for efficiency.
    /// </summary>
    /// <typeparam name="TEntity">The type of entity constructed or modified for each row of input data (e.g., a custom entity class).</typeparam>
    /// <typeparam name="TReturn">The type of the output collection that accumulates the processed entities (e.g., <see cref="List{TEntity}"/> or <see cref="Dictionary{TKey, TEntity}"/>).
    /// Must have a parameterless constructor.</typeparam>
    /// <param name="inputData">The list of dictionaries representing the raw data to process, where each dictionary contains key-value pairs
    /// for a single row (e.g., "Extend0_Id" mapped to a value).</param>
    /// <returns>The populated collection of type <typeparamref name="TReturn"/> containing the processed entities.</returns>
    /// <remarks>
    /// This method leverages expression trees to dynamically construct a processing pipeline, ensuring flexibility and performance.
    /// The <see cref="IterRowProcessors"/> collection must be defined elsewhere (e.g., as a field or property) and provide the logic
    /// for transforming each row into an entity and adding it to the output collection.
    /// </remarks>
    public TReturn ProcessData<TEntity, TReturn>(List<IDictionary<string, object>> inputData)
    {
        ...
        ParameterExpression iterationRowParameter = Expression.Parameter(typeof(IDictionary<string, object>), "iter");

        // Entity being constructed/modified for the current row
        ParameterExpression currentEntityVariable = Expression.Variable(typeof(TEntity), "currentEntity");

        // Collection that accumulates all processed entities
        ParameterExpression outputCollectionVariable = Expression.Variable(typeof(TReturn), "output");

        // Build the loop body, also assigns the current row from the enumerator to iterationRowParameter
        List<Expression> loopBodyExpressions = [Expression.Assign(iterationRowParameter, Expression.Property(itorVariable, "Current"))];
        ...
        LoopExpression whileLoopBody = Expression.Loop(
            Expression.IfThenElse(
                Expression.Call(itorVariable, ItorMoveNextMethod), // If MoveNext returns true (more rows),
                Expression.Block([iterationRowParameter, currentEntityVariable], loopBodyExpressions), // Execute the loop body with the current row
                Expression.Break(breakLabel)), // Otherwise, break out of the loop
            breakLabel); // Target label for breaking the loop

        TryExpression whileBlock = Expression.TryFinally(
        ...
        BlockExpression fullBlock = Expression.Block(
            ...
            [
                ...
                Expression.Assign(itorVariable, Expression.Call(Expression.Constant(inputData), GetEnumeratorForIEnumDictStrObj)),
